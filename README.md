...
API: `https://localhost:7259`, Swagger: `/swagger`.
...
Для Windows — `-f net8.0-windows10.0.19041.0`.

Адрес API задаётся в `MauiProgram.cs` при настройке `HttpClient`.
...
Все данные в проекте — вымышленные. Названия препаратов, ФИО сотрудников, номера серий и комментарии сгенерированы специально для демонстрации и не относятся к реальному производству.
